...
router.post('/api/saveRestaurantProfile', async (req, res) => {
  ...
  const update_res = `UPDATE restaurants set store_name = ? ,  store_location = ? , description = ? , restaurant_img = ?, cuisine = ?, timings = ? , delivery_type = ?, dishes_type = ?, phone = ?, street = ?, state = ?, country = ?, pincode = ? where owner_email= ?`
